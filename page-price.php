<?php get_header(); ?>

<main class="l-main">

  <section class="p-pagePrice">

    <div class="p-pageSitemap__head c-pageTitleHead">
      <?php get_template_part('template-parts/part', 'breadcrumb'); ?>
      <div class="c-pageTitle">
        <div class="c-pageTitle__en">PRICE</div>
        <h1 class="c-pageTitle__ja">料金一覧</h2>
      </div>
    </div>

    <div class="p-pagePrice__wrap">

      <div class="p-pageClinic__body">

        <ul class="p-pageCliniclink__btns">
            <li class="p-pageClinicLink__btn"><a href="#course">コースのご案内<div class="p-pageClinicLink__btnMark"></div></a></li>
            <li class="p-pageClinicLink__btn"><a href="#price">施術料金<div class="p-pageClinicLink__btnMark"></div></a></li>
        </ul>

      </div>

      <section id="course">
        <h2 class="p-pageClinicContent__title">コースのご案内</h2>
        
        <h3>トライアルボディメイクコース</h3>
        <div class="price__scroll-wrap js-scrollable">
        <table>
          <thead>
          <tr>
            <th>施術</th>
            <th>回数</th>
            <th>販売価格</th>
            <th>初回限定価格</th>
          </tr>
          </thead>
          <tbody>
          <tr>
            <td>CLATUUα　1部位</td>
            <td>1回</td>
            <td rowspan="3"><span class="price-normal">¥95,500</span></td>
            <td rowspan="3"><span class="price-trial">¥27,500</span></td>
          </tr>
          <tr>
            <td>ULTRAcel [zíː]</td>
            <td>1回</td>
          </tr>
          <tr>
            <td>脂肪溶解注射</td>
            <td>1回</td>
          </tr>
          </tbody>
        </table>
        </div>
        <h3>トライアル短期実感コース</h3>
        <div class="price__scroll-wrap js-scrollable">
        <table>
          <thead>
          <tr>
            <th>施術</th>
            <th>回数</th>
            <th>販売価格</th>
            <th>初回限定価格</th>
          </tr>
          </thead>
          <tbody>
          <tr>
            <td>CLATUUα　2部位</td>
            <td>3回</td>
            <td rowspan="3"><span class="price-normal">¥513,000</span></td>
            <td rowspan="3"><span class="price-trial">¥490,000</span></td>
          </tr>
          <tr>
            <td>食欲抑制ダイエット注射</td>
            <td>3本</td>
          </tr>
          <tr>
            <td>医師監修-管理栄養士による食事指導-</td>
            <td>3回</td>
          </tr>
          </tbody>
        </table>
        </div>
        <h3>ボディメイクコース</h3>
        <div class="price__scroll-wrap js-scrollable">
        <table>
          <thead>
          <tr>
            <th>施術</th>
            <th>回数</th>
            <th>販売価格</th>
            <th>初回限定価格</th>
          </tr>
          </thead>
          <tbody>
          <tr>
            <td>CLATUUα　2部位</td>
            <td>2回</td>
            <td rowspan="2"><span class="price-normal">¥560,000</span></td>
            <td rowspan="2"><span class="price-trial">¥400,000</span></td>
          </tr>
          <tr>
            <td>StimSure　2部位</td>
            <td>4回</td>
          </tr>
          </tbody>
        </table>
        </div>
        <h3>メディカルインセプションコース</h3>
        <div class="price__scroll-wrap js-scrollable">
        <table>
          <thead>
          <tr>
            <th>施術</th>
            <th>回数</th>
            <th>販売価格</th>
            <th>初回限定価格</th>
          </tr>
          </thead>
          <tbody>
          <tr>
            <td>肥満遺伝子検査</td>
            <td>1回</td>
            <td rowspan="9"><span class="price-normal">¥703,000</span></td>
            <td rowspan="9"><span class="price-trial">¥460,000</span></td>
          </tr>
          <tr>
            <td>CLATUUα　2部位</td>
            <td>1回</td>
          </tr>
          <tr>
            <td>StimSure　2部位</td>
            <td>2回</td>
          </tr>
          <tr>
            <td>ULTRAcel [zíː]</td>
            <td>1回</td>
          </tr>
          <tr>
            <td>ダイエット点滴</td>
            <td>2回</td>
          </tr>
          <tr>
            <td>エクソソーム点滴</td>
            <td>1回</td>
          </tr>
          <tr>
            <td>脂肪溶解注射</td>
            <td>1回</td>
          </tr>
          <tr>
            <td>Liend-リエートオリジナルダイエットプロテイン</td>
            <td>2袋</td>
          </tr>
          <tr>
            <td>Laloc-リエートオリジナルダイエットサプリ</td>
            <td>1箱</td>
          </tr>
          </tbody>
        </table>
        </div>
        <h3>メディカルシェイプコース</h3>
        <div class="price__scroll-wrap js-scrollable">
        <table>
          <thead>
          <tr>
            <th>施術</th>
            <th>回数</th>
            <th>販売価格</th>
            <th>初回限定価格</th>
          </tr>
          </thead>
          <tbody>
          <tr>
            <td>肥満遺伝子検査</td>
            <td>1回</td>
            <td rowspan="10"><span class="price-normal">¥1,779,000</span></td>
            <td rowspan="10"><span class="price-trial">¥750,000</span></td>
          </tr>
          <tr>
            <td>inbody計測</td>
            <td>10回</td>
          </tr>
          <tr>
            <td>StimSure　2部位</td>
            <td>8回</td>
          </tr>
          <tr>
            <td>ULTRAcel [zíː]</td>
            <td>1回</td>
          </tr>
          <tr>
            <td>脂肪溶解注射</td>
            <td>3回</td>
          </tr>
          <tr>
            <td>ダイエット美容点滴</td>
            <td>3回</td>
          </tr>
          <tr>
            <td>エクソソーム点滴</td>
            <td>3回</td>
          </tr>
          <tr>
            <td>医師監修-管理栄養士による食事指導-</td>
            <td>10回</td>
          </tr>
          <tr>
            <td>Liend-リエートオリジナルダイエットプロテイン</td>
            <td>6袋</td>
          </tr>
          <tr>
            <td>Laloc-リエートオリジナルダイエットサプリ</td>
            <td>3箱</td>
          </tr>
          </tbody>
        </table>
        </div>
        <h3>美容痩身メディカルコース（2ヶ月）</h3>
        <div class="price__scroll-wrap js-scrollable">
        <table>
          <thead>
          <tr>
            <th>施術</th>
            <th>回数</th>
            <th>販売価格</th>
            <th>初回限定価格</th>
          </tr>
          </thead>
          <tbody>
          <tr>
            <td>肥満遺伝子検査</td>
            <td>1回</td>
            <td rowspan="8"><span class="price-normal">¥1,318,000</span></td>
            <td rowspan="8"><span class="price-trial">¥868,000</span></td>
          </tr>
          <tr>
            <td>inbody計測</td>
            <td>6回</td>
          </tr>
          <tr>
            <td>CLATUUα　2部位</td>
            <td>2回</td>
          </tr>
          <tr>
            <td>StimSure　2部位</td>
            <td>6回</td>
          </tr>
          <tr>
            <td>脂肪溶解注射</td>
            <td>1回</td>
          </tr>
          <tr>
            <td>医師監修-管理栄養士による食事指導-</td>
            <td>6回</td>
          </tr>
          <tr>
            <td>ダイエット薬セット</td>
            <td>2ヶ月分</td>
          </tr>
          <tr>
            <td>Liend-リエートオリジナルダイエットプロテイン</td>
            <td>4袋</td>
          </tr>
          </tbody>
        </table>
        </div>
        <h3>美容痩身メディカルコース（3ヶ月）</h3>
        <div class="price__scroll-wrap js-scrollable">
        <table>
          <thead>
          <tr>
            <th>施術</th>
            <th>回数</th>
            <th>販売価格</th>
            <th>初回限定価格</th>
          </tr>
          </thead>
          <tbody>
          <tr>
            <td>肥満遺伝子検査</td>
            <td>1回</td>
            <td rowspan="8"><span class="price-normal">¥2,016,000</span></td>
            <td rowspan="8"><span class="price-trial">¥1,235,000</span></td>
          </tr>
          <tr>
            <td>inbody計測</td>
            <td>10回</td>
          </tr>
          <tr>
            <td>CLATUUα　2部位</td>
            <td>3回</td>
          </tr>
          <tr>
            <td>StimSure　2部位</td>
            <td>9回</td>
          </tr>
          <tr>
            <td>脂肪溶解注射</td>
            <td>2回</td>
          </tr>
          <tr>
            <td>医師監修-管理栄養士による食事指導-</td>
            <td>10回</td>
          </tr>
          <tr>
            <td>ダイエット薬セット（1ヶ月分）</td>
            <td>3ヶ月分</td>
          </tr>
          <tr>
            <td>Liend-リエートオリジナルダイエットプロテイン</td>
            <td>6袋</td>
          </tr>
          </tbody>
        </table>
        </div>
        <h3>美容痩身メディカルスリムコース（2ヶ月）</h3>
        <div class="price__scroll-wrap js-scrollable">
        <table>
          <thead>
          <tr>
            <th>施術</th>
            <th>回数</th>
            <th>販売価格</th>
            <th>初回限定価格</th>
          </tr>
          </thead>
          <tbody>
          <tr>
            <td>肥満遺伝子検査</td>
            <td>1回</td>
            <td rowspan="12"><span class="price-normal">¥1,420,000</span></td>
            <td rowspan="12"><span class="price-trial">¥957,000</span></td>
          </tr>
          <tr>
            <td>inbody計測</td>
            <td>6回</td>
          </tr>
          <tr>
            <td>CLATUUα　2部位</td>
            <td>2回</td>
          </tr>
          <tr>
            <td>StimSure　2部位</td>
            <td>4回</td>
          </tr>
          <tr>
            <td>RADIO STIM PRO</td>
            <td>2回</td>
          </tr>
          <tr>
            <td>脂肪溶解注射</td>
            <td>2回</td>
          </tr>
          <tr>
            <td>ダイエット点滴</td>
            <td>4回</td>
          </tr>
          <tr>
            <td>ダイエット薬セット（1ヶ月分）</td>
            <td>2ヶ月分</td>
          </tr>
          <tr>
            <td>Liend-リエートオリジナルダイエットプロテイン</td>
            <td>4袋</td>
          </tr>
          <tr>
            <td>Laloc-リエートオリジナルダイエットサプリ</td>
            <td>2箱</td>
          </tr>
          <tr>
            <td>医師監修-管理栄養士による食事指導-</td>
            <td>6回</td>
          </tr>
          </tbody>
          </table>
          <h3>美容痩身メディカルスリムコース（3ヶ月）</h3>
          <div class="price__scroll-wrap js-scrollable">
          <table>
          <thead>
          <tr>
            <th>施術</th>
            <th>回数</th>
            <th>販売価格</th>
            <th>初回限定価格</th>
          </tr>
          </thead>
          <tbody>
          <tr>
            <td>肥満遺伝子検査</td>
            <td>1回</td>
            <td rowspan="11"><span class="price-normal">¥2,134,000</span></td>
            <td rowspan="11"><span class="price-trial">¥1,387,100</span></td>
          </tr>
          <tr>
            <td>inbody計測</td>
            <td>10回</td>
          </tr>
          <tr>
            <td>CLATUUα　2部位</td>
            <td>3回</td>
          </tr>
          <tr>
            <td>StimSure　2部位</td>
            <td>6回</td>
          </tr>
          <tr>
            <td>RADIO STIM PRO</td>
            <td>3回</td>
          </tr>
          <tr>
            <td>脂肪溶解注射</td>
            <td>3回</td>
          </tr>
          <tr>
            <td>ダイエット点滴</td>
            <td>6回</td>
          </tr>
          <tr>
            <td>ダイエット薬セット（1ヶ月分）</td>
            <td>3ヶ月分</td>
          </tr>
          <tr>
            <td>Liend-リエートオリジナルダイエットプロテイン</td>
            <td>6袋</td>
          </tr>
          <tr>
            <td>Laloc-リエートオリジナルダイエットサプリ</td>
            <td>3箱</td>
          </tr>
          <tr>
            <td>医師監修-管理栄養士による食事指導-</td>
            <td>10回</td>
          </tr>
          </tbody>
        </table>
        </div>
        <h3>美容痩身メディカルリプロコース（2ヶ月）</h3>
        <div class="price__scroll-wrap js-scrollable">
        <table>
          <thead>
          <tr>
            <th>施術</th>
            <th>回数</th>
            <th>販売価格</th>
            <th>初回限定価格</th>
          </tr>
          </thead>
          <tbody>
          <tr>
            <td>肥満遺伝子検査</td>
            <td>1回</td>
            <td rowspan="13"><span class="price-normal">¥1,810,000</span></td>
            <td rowspan="13"><span class="price-trial">¥1,166,200</span></td>
          </tr>
          <tr>
            <td>inbody計測</td>
            <td>6回</td>
          </tr>
          <tr>
            <td>CLATUUα　2部位</td>
            <td>2回</td>
          </tr>
          <tr>
            <td>StimSure　2部位</td>
            <td>4回</td>
          </tr>
          <tr>
            <td>ULTRAcel [zíː]</td>
            <td>2回</td>
          </tr>
          <tr>
            <td>RADIO STIM PRO</td>
            <td>1回</td>
          </tr>
          <tr>
            <td>脂肪溶解注射</td>
            <td>2回</td>
          </tr>
          <tr>
            <td>エクソソーム点滴</td>
            <td>2回</td>
          </tr>
          <tr>
            <td>ダイエット美容点滴</td>
            <td>4回</td>
          </tr>
          <tr>
            <td>ダイエット薬セット（1ヶ月分）</td>
            <td>2ヶ月分</td>
          </tr>
          <tr>
            <td>Liend-リエートオリジナルダイエットプロテイン</td>
            <td>4袋</td>
          </tr>
          <tr>
            <td>Laloc-リエートオリジナルダイエットサプリ</td>
            <td>2箱</td>
          </tr>
          <tr>
            <td>医師監修-管理栄養士による食事指導-</td>
            <td>6回</td>
          </tr>
          </tbody>
        </table>
        </div>
        <h3>美容痩身メディカルリプロコース（3ヶ月）</h3>
        <div class="price__scroll-wrap js-scrollable">
        <table>
          <thead>
          <tr>
            <th>施術</th>
            <th>回数</th>
            <th>販売価格</th>
            <th>初回限定価格</th>
          </tr>
          </thead>
          <tbody>
          <tr>
            <td>肥満遺伝子検査</td>
            <td>1回</td>
            <td rowspan="13"><span class="price-normal">¥2,754,000</span></td>
            <td rowspan="13"><span class="price-trial">¥1,566,000</span></td>
          </tr>
          <tr>
            <td>inbody計測</td>
            <td>10回</td>
          </tr>
          <tr>
            <td>CLATUUα　2部位</td>
            <td>3回</td>
          </tr>
          <tr>
            <td>StimSure　2部位</td>
            <td>6回</td>
          </tr>
          <tr>
            <td>ULTRAcel [zíː]</td>
            <td>3回</td>
          </tr>
          <tr>
            <td>RADIO STIM PRO</td>
            <td>1回</td>
          </tr>
          <tr>
            <td>脂肪溶解注射</td>
            <td>3回</td>
          </tr>
          <tr>
            <td>エクソソーム点滴</td>
            <td>3回</td>
          </tr>
          <tr>
            <td>ダイエット美容点滴</td>
            <td>6回</td>
          </tr>
          <tr>
            <td>ダイエット薬セット（1ヶ月分）</td>
            <td>3ヶ月分</td>
          </tr>
          <tr>
            <td>Liend-リエートオリジナルダイエットプロテイン</td>
            <td>6袋</td>
          </tr>
          <tr>
            <td>Laloc-リエートオリジナルダイエットサプリ</td>
            <td>3箱</td>
          </tr>
          <tr>
            <td>医師監修-管理栄養士による食事指導-</td>
            <td>10回</td>
          </tr>
          </tbody>
        </table>
        </div>

      </section>
      <section id="price">
        <h2 class="p-pageClinicContent__title">施術料金</h2>

        <h3>医療機器施術</h3>
        <table>
          <thead>
            <tr>
            <th>施術内容/回数</th>
            <th>1回</th>
            <th>3回</th>
            <th>5回</th>
            </tr>
          </thead>
          <tbody>
          <tr>
            <td>inbody（1回）</td>
            <td>¥500</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>CLATUUα（2部位）</td>
            <td>¥100,000</td>
            <td>¥255,000</td>
            <td>¥375,000</td>
            </tr>
            <tr>
            <td>StimSure（2部位）</td>
            <td>¥90,000</td>
            <td>¥225,000</td>
            <td>¥325,000</td>
            </tr>
            <tr>
            <td>ULTRAcel [zíː] 全顔</td>
            <td>¥140,000</td>
            <td>¥390,000</td>
            <td>¥600,000</td>
            </tr>
            <tr>
            <td>ULTRAcel [zíː] ボディ</td>
            <td>¥210,000</td>
            <td>¥465,000</td>
            <td>¥725,000</td>
            </tr>
            <tr>
            <td>ULTRAcel [zíː] 目元</td>
            <td>¥52,500</td>
            <td>¥153,000</td>
            <td>¥245,000</td>
            </tr>
            <tr>
            <td>ULTRAcel [zíː] 首</td>
            <td>¥70,000</td>
            <td>¥204,000</td>
            <td>¥330,000</td>
            </tr>
            <tr>
            <td>ULTRAcel [zíː] 顎下</td>
            <td>¥70,000</td>
            <td>¥204,000</td>
            <td>¥330,000</td>
            </tr>
            <tr>
            <td>RADIO STIM PRO</td>
            <td>70,000</td>
            <td>¥204,000</td>
            <td>¥330,000</td>
            </tr>
          </tbody>
        </table>

        <h3>内服薬・外用薬</h3>
        <table>
          <thead>
            <tr>
            <th>内服薬・外用薬</th>
            <th>1回</th>
            <th>2回</th>
            <th>3回</th>
            </tr>
          </thead>
          <tbody>
          <tr>
            <td>ダイエット薬セット（1ヶ月）</td>
            <td>¥200,000</td>
            <td>¥390,000</td>
            <td>¥570,000</td>
            </tr>
            <tr>
            <td>サクセンダ（1本）</td>
            <td>¥33,000</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>リベルサス 3mg（30日分）</td>
            <td>¥16,500</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>リベルサス 7mg（30日分）</td>
            <td>¥38,500</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>リベルサス 14mg（30日分）</td>
            <td>¥57,500</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>オゼンピック 2.0mg（1本）</td>
            <td>¥58,000</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>SGLT2-カナグル（30日分）</td>
            <td>¥23,500</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>ゼニカル（30日分）</td>
            <td>¥12,750</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>トランサミン250㎎（30日分）</td>
            <td>¥1,100</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>酸化マグネシウム（30日分）</td>
            <td>¥550</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>五苓散（30日分）</td>
            <td>¥9,000</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>マンジャロ2.5mg（2週間分）</td>
            <td>¥33,000</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>マンジャロ2.5mg（4週間分）</td>
            <td>¥63,000</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>マンジャロ5mg（2週間分）</td>
            <td>¥58,000</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>マンジャロ5mg（4週間分）</td>
            <td>¥100,000</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>マンジャロ7.5mg（2週間分）</td>
            <td>¥100,000</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>マンジャロ7.5mg（4週間分）</td>
            <td>¥180,000</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>マンジャロ10mg（2週間分）</td>
            <td>¥120,000</td>
            <td>-</td>
            <td>-</td>
            </tr>
            <tr>
            <td>マンジャロ10mg（4週間分）</td>
            <td>¥220,000</td>
            <td>-</td>
            <td>-</td>
            </tr>
          </tbody>
        </table>

        <h3>オリジナルサプリメント</h3>
        <table>
          <thead>
            <tr>
            <th>施術内容/回数</th>
            <th>1回</th>
            </tr>
          </thead>
          <tbody>
            <tr>
            <td>Liend-リエートオリジナルダイエットプロテイン（1袋）</td>
            <td>¥6,000</td>
            </tr>
            <tr>
            <td>Laloc-リエートオリジナルダイエットサプリ（1箱）</td>
            <td>¥16,000</td>
            </tr>
          </tbody>
        </table>

        <h3>ダイエット注射・美容点滴</h3>
        <div class="price__scroll-wrap js-scrollable">
        <table>
          <thead>
            <tr>
            <th>施術内容/回数</th>
            <th>1回</th>
            <th>2回</th>
            <th>3回</th>
            <th>4回</th>
            <th>5回</th>
            </tr>
          </thead>
          <tbody>
            <tr>
            <td>脂肪溶解注射</td>
            <td>¥70,000</td>
            <td>-</td>
            <td>¥204,000</td>
            <td>-</td>
            <td>¥330,000</td>
            </tr>
            <tr>
            <td>プラセンタ注射</td>
            <td>¥3,500</td>
            <td>-</td>
            <td>¥9,600</td>
            <td>-</td>
            <td>¥14,500</td>
            </tr>
            <tr>
            <td>エクソソーム点滴</td>
            <td>¥150,000</td>
            <td>¥280,000</td>
            <td>¥420,000</td>
            <td>¥480,000</td>
            <td>¥550,000</td>
            </tr>
            <tr>
            <td>ダイエット美容点滴</td>
            <td>¥15,000</td>
            <td>-</td>
            <td>¥42,000</td>
            <td>¥54,000</td>
            <td>¥65,000</td>
            </tr>
            <tr>
            <td>２日酔い改善点滴</td>
            <td>¥5,500</td>
            <td>-</td>
            <td>¥15,600</td>
            <td>-</td>
            <td>¥24,500</td>
            </tr>
            <tr>
            <td>肌荒れ改善点滴</td>
            <td>¥7,500</td>
            <td>-</td>
            <td>¥21,000</td>
            <td>-</td>
            <td>¥33,500</td>
            </tr>
            <tr>
            <td>疲労回復注射</td>
            <td>¥7,500</td>
            <td>-</td>
            <td>¥21,000</td>
            <td>-</td>
            <td>¥33,500</td>
            </tr>
            <tr>
            <td>美白点滴</td>
            <td>¥7,500</td>
            <td>-</td>
            <td>¥21,000</td>
            <td>-</td>
            <td>¥33,500</td>
            </tr>
          </tbody>
        </table>
        </div>
        
        <h3>遺伝子検査</h3>
        <table>
          <thead>
            <tr>
            <th>施術内容/回数</th>
            <th>1回</th>
            </tr>
          </thead>
          <tbody>
            <tr>
            <td>遺伝子検査</td>
            <td>¥15,000</td>
            </tr>
          </tbody>
        </table>

        <h3>栄養・食事指導</h3>
        <table>
          <thead>
            <tr>
            <th>施術内容/回数</th>
            <th>1回</th>
            </tr>
          </thead>
          <tbody>
            <tr>
            <td>医師監修-管理栄養士による食事指導-</td>
            <td>¥11,000</td>
            </tr>
          </tbody>
        </table>

      </section>

      <p class="p-singleMenu__text">※コース以外についてはコース終了後に継続いただいた方のみのご案内となっております。</p>
    
    </div>

  </section>

</main>

<?php get_footer();
